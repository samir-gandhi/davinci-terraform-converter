...
func (c *ExportCommand) Run(args []string, logger grpc.Logger) error {
	...
	return c.runExport(logger, *services, *workerEnvironmentID, *exportEnvironmentID, *regionCode, *clientID, *clientSecret, *out, *skipDependencies, !*skipImports)
}

// runExport handles API export of all resources from an environment
func (c *ExportCommand) runExport(logger grpc.Logger, services []string, workerEnvironmentID, exportEnvironmentID, regionCode, clientID, clientSecret, out string, skipDeps bool, generateImports bool) error {
	...
	// Export all resources with options
	hcl, err := exporter.ExportEnvironmentWithOptions(ctx, client, exporter.ExportOptions{
